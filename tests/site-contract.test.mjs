...
test("首页包含 v1.3.3 双平台下载入口、unsigned 提示及完整产品信息", () => {
  ...
  assert.equal(packageManifest.version, "1.3.3");
  ...
  assert.match(
    html,
    /https:\/\/github\.com\/SilnoGM\/SuperSent-Releases\/releases\/download\/v1\.3\.3\/SuperSent-1\.3\.3\.dmg/
  );
  assert.match(
    html,
    /https:\/\/github\.com\/SilnoGM\/SuperSent-Releases\/releases\/download\/v1\.3\.3\/SuperSent-1\.3\.3-windows-x64-Setup\.msi/
  );
  ...
  assert.doesNotMatch(
    html,
    /https:\/\/github\.com\/SilnoGM\/SuperSent-Releases\/releases\/download\/v1\.3\.3\/SuperSent-1\.3\.3-windows-x64-Setup\.exe/
  );
  assert.doesNotMatch(
    html,
    /https:\/\/github\.com\/SilnoGM\/SuperSent-Releases\/releases\/download\/v1\.3\.3\/SuperSent-1\.3\.3-windows-x64-Portable\.zip/
  );
  ...
  assert.match(html, /href="releases\.html#v1-3-3"[^>]*>查看发布说明<\/a>/);
...
test("版本更新时间线从 v1.3.0 开始并只展示双平台版本", () => {
  ...
  assert.match(html, /<time datetime="2026-08-25">2026 年 8 月 25 日<\/time>/);
  ...
  assert.match(html, /任务栏[^<]*品牌图标/);
  assert.match(html, /选择安装目录/);
  assert.match(html, /<dt>4<\/dt><dd>双平台版本<\/dd>/);
  assert.match(html, /id="v1-3-3" class="release-node release-node-current"/);
...
test("所有版本说明入口留在官网内且保留外部下载边界", () => {
  ...
  assert.match(pages[0], /href="releases\.html#v1-3-3"[^>]*>查看发布说明<\/a>/);
  assert.match(pages[0], /href="releases\.html"[^>]*>所有版本<\/a>/);
  assert.match(pages[1], /href="releases\.html"[^>]*>版本更新<\/a>/);
  assert.match(pages[2], /href="releases\.html#v1-3-3"[^>]*>发布说明<\/a>/);
  assert.doesNotMatch(
    combinedPages,
  ...
  assert.match(combinedPages, /SuperSent-Releases\/releases\/download\/v1\.3\.3/);
